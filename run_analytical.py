...
def run_chunk(start, end):
    ...
    subprocess.run(["solver_analytical.exe", str(start), str(end)])
...
all_files = glob.glob("results/analytical_data_chunk_*.csv")
df_list = []
for f in all_files:
    df_list.append(pd.read_csv(f))
    
combined = pd.concat(df_list, ignore_index=True)
combined.to_csv("results/analytical_data.csv", index=False)
