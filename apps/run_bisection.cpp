...
void save_trace(CanonicalPot& pot, double phi_0, double eps, const std::string& fname) {
    ...
    while (r < 1e12 && step_cnt < 100000) {
        ...
        if (step_cnt % 10 == 0 || y.phi < 0 || y.dphi > 0 || std::isnan(y.phi) || r > 1e10) {
            trace << std::scientific << r << "," << y.phi << "," << y.dphi << "," << pot.V(y.phi) << "," << pot.dV(y.phi) << "\n";
        }
...
int main(int argc, char* argv[]) {
    double Mh = 5.0, Mt = 105.0;
    double c6 = 0.0;
    
    std::cout << "Canonical Solver Bisection: Mh=" << Mh << ", Mt=" << Mt << ", c6=" << c6 << std::endl;
    ...
    double phi_low = 2.325711e+004; // UNDERSHOOT
    double phi_high = 2.742683e+004; // OVERSHOOT
    double eps = 1e-3;
    
    std::cout << "Starting Bisection..." << std::endl;
    double phi_opt = 0;
    TrajectoryResult best_res;
    
    for (int iter = 0; iter < 100; ++iter) {
        phi_opt = std::exp(0.5 * (std::log(phi_low) + std::log(phi_high)));
        TrajectoryResult res = shoot(pot, phi_opt, eps);
        
        std::cout << "Iter " << iter << " | phi_opt = " << std::scientific << phi_opt 
                  << " | status = " << status_to_string(res.status) 
                  << " | max_r = " << res.max_r << std::endl;
        ...
        if (std::abs(phi_high - phi_low) / phi_opt < 1e-15) {
            std::cout << "Bisection converged to float precision." << std::endl;
            break;
        }
    }
    
    std::cout << "\n======================================" << std::endl;
    std::cout << "Converged Shooting Parameter: phi_0 = " << std::scientific << phi_opt << std::endl;
    std::cout << "S_kin = " << best_res.S_kin << std::endl;
    std::cout << "S_pot = " << best_res.S_pot << std::endl;
    std::cout << "S_E (S_kin + S_pot) = " << (best_res.S_kin + best_res.S_pot) << std::endl;
    std::cout << "Virial Ratio (S_kin / |S_pot|) = " << std::abs(best_res.S_kin / best_res.S_pot) << std::endl;
    std::cout << "Equation of Motion Residual (S_kin + 2*S_pot) = " << (best_res.S_kin + 2.0*best_res.S_pot) << std::endl;
    std::cout << "Final phi(r_max) = " << best_res.final_phi << std::endl;
    std::cout << "======================================\n" << std::endl;
    
    save_trace(pot, 2.325711e+004, eps, "data/bisection_undershoot.csv");
    save_trace(pot, phi_opt, eps, "data/bisection_converged.csv");
    save_trace(pot, 2.742683e+004, eps, "data/bisection_overshoot.csv");
