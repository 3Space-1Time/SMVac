...
private:
    ...
    bool sorted = false;
public:
    void clear() { table.clear(); sorted = false; }
    void add_point(double t, const Params& p) { table.push_back({t, p}); sorted = false; }
    void sort_table() {
        if(!sorted) {
            std::sort(table.begin(), table.end(), [](const pair<double, Params>& a, const pair<double, Params>& b) {
...
struct TrajectoryResult {
    ...
    int step_cnt;
    double taylor_corr;
    double first_step_phi;
    double first_step_dphi;
    bool overflow;
    double max_residual;
};

TrajectoryResult shoot(const CanonicalPot& pot, double phi_0, double epsilon, double tol_dr=0.01, int max_steps=1000000) {
    if (phi_0 <= 0) return {FAILED, 0, 0, phi_0, 0, 0, 0, 0, 0, 0, false, 0};
    
    // Scaled Taylor expansion at r = epsilon / phi_0
    double r = epsilon / phi_0;
    double dV0 = pot.dV(phi_0);
    double taylor_corr = 0.125 * r * r * dV0;
    
    State y;
    y.phi = phi_0 + taylor_corr;
    y.dphi = 0.25 * r * dV0;
    
    double S_kin = 0.0;
    double S_pot = 0.0;
    
    double dr = r; // Set initial step to the same scale as starting radius
    int step_cnt = 0;
    ...
    State y_prev = y;
    double prev_dphi = y.dphi;
    
    while (r < 1e12) {
        step_cnt++;
        if (step_cnt > max_steps) {
            return {MAX_STEPS, S_kin, S_pot, y.phi, y.dphi, r, step_cnt, taylor_corr, first_phi, first_dphi, overflow, max_residual};
        }
        ...
        if (step_cnt == 1) {
            first_phi = y_next.phi;
            first_dphi = y_next.dphi;
        }
        ...
        // Compute numerical residual phi'' + 3/r phi' - V'
        double d2phi_num = (y_next.dphi - y.dphi) / dr;
        double dV_eval = pot.dV(y.phi);
        double resid = std::abs(d2phi_num + (3.0/r)*y.dphi - dV_eval);
        if (resid > max_residual && std::isfinite(resid)) max_residual = resid;
        
        y = y_next;
        r += dr;
        
        // Termination Checks
        if (std::isnan(y.phi) || std::isnan(y.dphi) || std::isinf(y.phi) || std::isinf(y.dphi)) {
            overflow = true;
            return {FAILED, S_kin, S_pot, y.phi, y.dphi, r, step_cnt, taylor_corr, first_phi, first_dphi, overflow, max_residual};
        }
        if (y.phi < 0) {
            return {OVERSHOOT, S_kin, S_pot, y.phi, y.dphi, r, step_cnt, taylor_corr, first_phi, first_dphi, overflow, max_residual}; 
        }
        if (y.dphi > 0) {
            if (y.phi < 1e-2) {
                return {CONVERGED, S_kin, S_pot, y.phi, y.dphi, r, step_cnt, taylor_corr, first_phi, first_dphi, overflow, max_residual};
            }
            return {UNDERSHOOT, S_kin, S_pot, y.phi, y.dphi, r, step_cnt, taylor_corr, first_phi, first_dphi, overflow, max_residual}; 
        }
        
        // Adaptive Step Sizing
        double dr_target = tol_dr * std::abs(y.phi / (y.dphi + 1e-30));
        dr = std::max(1e-12, std::min(dr_target, 0.1 * r)); // Bound the step size to not explode
    }
...
int main(int argc, char* argv[]) {
    ...
    Params y_match = get_nnlo_matching(Mh, Mt);
    ...
    // Integrate backwards to t_start
    ...
    // Sort RGE table just in case it's out of order from backward/forward
    // RGEHelper handles internal sorting if we expose it, but wait, add_point just appends!
    // I should fix add_point in RGEHelper to sort before use!
    
    CanonicalPot pot = {c6, &rge};
    
    // Find the interval!
    double phi_barrier = -1;
    double phi_min = -1;
    
    for(double phi = 10.0; phi < 1e8; phi *= 1.05) {
        double dV = pot.dV(phi);
        if (phi_barrier < 0 && dV < 0) {
            phi_barrier = phi;
        }
        if (phi_barrier > 0 && phi_min < 0 && dV > 0) {
            phi_min = phi;
        }
    }
    
    std::cout << "Barrier (dV turns negative): " << phi_barrier << std::endl;
    std::cout << "Minimum (dV turns positive): " << phi_min << std::endl;
    
    // Sweep the interval!
    std::ofstream summary("data/sweep_summary.csv");
    summary << "phi_0,status,S_kin,S_pot,max_r,step_cnt\n";
    
    double eps = 1e-3;
    
    // Select a few to save full traces
    std::vector<double> trace_phis;
    if (phi_barrier > 0 && phi_min > 0) {
        trace_phis.push_back(phi_barrier * 1.5);
        trace_phis.push_back(std::exp(0.5*(std::log(phi_barrier) + std::log(phi_min))));
        trace_phis.push_back(phi_min * 0.8);
    } else {
        trace_phis.push_back(1e4);
        trace_phis.push_back(1e5);
    }
    
    double phi0_start = phi_barrier > 0 ? phi_barrier : 1e3;
    double phi0_end = phi_min > 0 ? phi_min : 1e6;
    
    for (int i = 0; i <= 50; i++) {
        double log_p = std::log(phi0_start) + i * (std::log(phi0_end) - std::log(phi0_start)) / 50.0;
        double phi0 = std::exp(log_p);
        
        TrajectoryResult r = shoot(pot, phi0, eps);
        summary << std::scientific << phi0 << "," << status_to_string(r.status) << ","
                << r.S_kin << "," << r.S_pot << "," << r.max_r << "," << r.step_cnt << "\n";
    }
    summary.close();
    ...
    for (size_t i = 0; i < trace_phis.size(); i++) {
        ...
        trace << std::scientific << r << "," << y.phi << "," << y.dphi << "," << pot.V(y.phi) << "," << pot.dV(y.phi) << "\n";
        
        while (r < 1e12 && step_cnt < 100000) {
            step_cnt++;
            auto derivs = [&](double curr_r, State curr_y) -> State {
                double V_val = pot.V(curr_y.phi);
                double dV_val = pot.dV(curr_y.phi);
            ...
            if (step_cnt % 10 == 0 || y.phi < 0 || y.dphi > 0 || std::isnan(y.phi)) {
                trace << std::scientific << r << "," << y.phi << "," << y.dphi << "," << pot.V(y.phi) << "," << pot.dV(y.phi) << "\n";
            }
            
            if (std::isnan(y.phi) || std::isnan(y.dphi) || std::isinf(y.phi) || std::isinf(y.dphi)) break;
            if (y.phi < 0) break; // Overshoot
            if (y.dphi > 0) {
                if (y.phi < 1e-2) {
                    // Converged!
                }
            ...
            dr = std::max(1e-12, std::min(dr_target, 0.1 * r));
        }
        trace.close();
        std::cout << "Dumped trace for phi0 = " << phi_0 << " to " << fname << std::endl;
    }
