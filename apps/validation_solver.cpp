...
double shoot(PotentialInterp& pot, double phi_0, double& S_kin, double& S_pot, double& R_half, double tol_dr=0.01, double max_r=1e8) {
    ...
    while (r < max_r) {
        ...
        dr = std::max(1e-5, std::min(dr_target, 1.0));
    }
    return y.phi;
}
...
int main() {
    double Mh = 5.0;
    std::vector<double> Mts = {95.0, 100.0, 102.0, 104.0, 105.0, 106.0, 108.0, 110.0, 115.0, 120.0};
    
    std::ofstream fout("data/ode_validation_raw.csv");
    fout << "Mt,FL_Phase,FL_S_total,FL_S_kin,FL_S_pot,ODE_Phase,ODE_S_total,ODE_S_kin,ODE_S_pot,phi_0,R_half\n";
    
    std::ofstream fconv("data/ode_convergence_raw.csv");
    fconv << "Mt,Test,Value,S_total\n";
    
    for (double Mt : Mts) {
        std::cout << "Analyzing Mt = " << Mt << std::endl;
        
        // 1. FL Calculation
        auto res = classify_stability(Mh, Mt);
        int fl_phase = std::get<0>(res);
        double fl_s_total = std::get<1>(res);
        
        // Recompute FL kinetic and potential components manually because classify_stability only returns total
        Params y = get_nnlo_matching(Mh, Mt);
        ...
        double fl_s_kin = 0;
        double fl_s_pot = 0;
        if (fl_phase == 2 || fl_phase == 3) {
            // Find mu_inst and min_lambda
            double t_min = 0, lam_min = 1.0, mu_inst = MPlanck;
            for(auto p : rge.table) { // hacky but fast enough
                double lam = get_lambda_eff(p.second);
                if (lam < lam_min) { lam_min = lam; t_min = p.first; }
            }
            if (lam_min < 0) {
                mu_inst = std::exp(t_min/2.0);
                double kinetic_term = (16.0 * pi * pi) / (3.0 * std::abs(lam_min));
                fl_s_kin = kinetic_term;
                fl_s_pot = fl_s_total - kinetic_term;
            }
        }
        ...
        while(phi_high < 1e18) { 
            double r = shoot(pot, phi_high, dummy1, dummy2, dummy3);
            if (r < 0) {
        ...
        int ode_phase = -1;
        double ode_s_total = -1, ode_s_kin = -1, ode_s_pot = -1, phi_opt = -1, r_half = -1;
        
        if (!found_overshoot) {
            ode_phase = 1; // Stable (failed to overshoot)
            ode_s_total = 1e100; // Infinity
        } else {
            for (int iter = 0; iter < 55; ++iter) {
                phi_opt = std::exp(0.5 * (std::log(phi_low) + std::log(phi_high)));
                double end_val = shoot(pot, phi_opt, ode_s_kin, ode_s_pot, r_half, 0.01, 1e8);
                if (end_val < 0) phi_high = phi_opt;
                else phi_low = phi_opt;
                if ((phi_high - phi_low) / phi_opt < 1e-14) break;
            }
            ode_s_total = ode_s_kin + ode_s_pot;
            
            if (ode_s_total > 400.0) ode_phase = 2; // Metastable
            else ode_phase = 3; // Unstable
        }
        
        fout << Mt << "," << fl_phase << "," << fl_s_total << "," << fl_s_kin << "," << fl_s_pot << "," 
             << ode_phase << "," << ode_s_total << "," << ode_s_kin << "," << ode_s_pot << "," 
             << phi_opt << "," << r_half << "\n";
             
        // 3. Convergence tests for 104, 105, 110
        if (Mt == 104 || Mt == 105 || Mt == 110) {
            if (found_overshoot) {
                // vary dr
                double k1, p1, r1;
                shoot(pot, phi_opt, k1, p1, r1, 0.05, 1e8);
                fconv << Mt << ",tol_dr,0.05," << (k1+p1) << "\n";
                shoot(pot, phi_opt, k1, p1, r1, 0.005, 1e8);
                fconv << Mt << ",tol_dr,0.005," << (k1+p1) << "\n";
                // vary max_r
                shoot(pot, phi_opt, k1, p1, r1, 0.01, 1e6);
                fconv << Mt << ",max_r,1e6," << (k1+p1) << "\n";
                shoot(pot, phi_opt, k1, p1, r1, 0.01, 1e9);
                fconv << Mt << ",max_r,1e9," << (k1+p1) << "\n";
            } else {
                fconv << Mt << ",tol_dr,0.05,-1\n";
                fconv << Mt << ",tol_dr,0.005,-1\n";
                fconv << Mt << ",max_r,1e6,-1\n";
                fconv << Mt << ",max_r,1e9,-1\n";
            }
        }
    }
    fout.close();
