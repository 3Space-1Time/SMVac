...
struct ActionComponents {
    ...
    double R_opt;
    bool valid;
    ActionComponents() : S_kin(-1), S_pot(-1), S_total(-1), lambda_R(0), R_opt(0), valid(false) {}
    ActionComponents(double sk, double sp, double st, double lr, double ro) 
        : S_kin(sk), S_pot(sp), S_total(st), lambda_R(lr), R_opt(ro), valid(true) {}
};
...
// DIAGNOSTIC VERSION: returns (S_kin, S_pot, S_total, lambda_R)
ActionComponents evaluate_action_components(RGEHelper& rge, double mu_inst, double R) {
    if (R <= 0.0) return ActionComponents();
    double mu_R = mu_inst / R;
    if (mu_R <= 0.0) return ActionComponents();
    double t_R    = 2.0 * std::log(mu_R);
    Params p_R    = rge.get_params(t_R);
    double lambda_R = p_R.lambda;
    if (lambda_R >= 0.0) return ActionComponents();
    double abs_lam  = std::abs(lambda_R);
    double prefactor = std::sqrt(2.0 / abs_lam);
    double kinetic_term = (16.0 * pi * pi) / (3.0 * abs_lam);
    if (kinetic_term > 5e5) return ActionComponents(kinetic_term, 0.0, kinetic_term, lambda_R, R);
    const int    N  = N_QUAD;
    ...
    return ActionComponents(kinetic_term, potential_integral, kinetic_term + potential_integral, lambda_R, R);
}

ActionComponents find_minimum_action_components(RGEHelper& rge, double mu_inst, double t_min_lambda) {
    const double invphi = (sqrt(5.0) - 1.0) / 2.0;
    const double invphi2 = (3.0 - sqrt(5.0)) / 2.0;
    
    double mu_opt = exp(t_min_lambda / 2.0);
    double R_opt_guess = mu_inst / mu_opt;
    double logR_opt = log(R_opt_guess);
    double a = logR_opt - 8.0;  
    double b = logR_opt + 8.0;  
    double tol = 1e-13;
    
    double h = b - a;
    if (h <= tol) return evaluate_action_components(rge, mu_inst, exp(0.5*(a+b)));
    ...
    double yc = evaluate_action_components(rge, mu_inst, exp(c)).S_total;
    double yd = evaluate_action_components(rge, mu_inst, exp(d)).S_total;
    ...
    for (int k = 0; k < n; ++k) {
        if (yc < yd) {
            ...
            h = invphi * h;
            c = a + invphi2 * h;
            yc = evaluate_action_components(rge, mu_inst, exp(c)).S_total;
        } else {
            a = c;
            c = d;
            yc = yd;
            h = invphi * h;
            d = a + invphi * h;
            yd = evaluate_action_components(rge, mu_inst, exp(d)).S_total;
        }
    }
    
    double min_R = exp(0.5 * (a + b));
    return evaluate_action_components(rge, mu_inst, min_R);
}

// --- CLASSIFICATION (DIAGNOSTIC) ---
std::tuple<int, double, double, double, double, double, double, double> 
classify_stability_diagnostic(double Mh, double Mt) {
    ...
    while (t < tPlanck) {
        ...
        if (std::abs(y.lambda) > 4*pi || y.yt > 4*pi) 
            return std::make_tuple(4, -1.0, -1.0, Mh_calc, -1.0, -1.0, 0.0, 0.0);
    ...
    if (!is_unstable) return std::make_tuple(1, -1.0, -1.0, Mh_calc, -1.0, -1.0, 0.0, 0.0);
    
    double S_approx = 8.0 * pi * pi / (3.0 * std::abs(min_lambda_eff));
    ActionComponents ac = find_minimum_action_components(rge, mu_inst, t_min_lambda);
    double S_exact = ac.S_total;
    
    double Tv = 1.179e44 / v;
    double S_threshold = 4.0 * std::log(Tv * mu_inst);
    
    int stab;
    if (S_exact > S_threshold) stab = 2;
    else stab = 3;
    
    return std::make_tuple(stab, S_exact, S_approx, Mh_calc, ac.S_kin, ac.S_pot, ac.lambda_R, ac.R_opt);
}
...
int main(int argc, char* argv[]) {
    ...
    // Target the sandwiched region: M_h ∈ [0, 100], M_t ∈ [90, 200]
    // Finer M_h spacing (0.5 GeV) and standard M_t spacing (0.25 GeV)
    vector<pair<double,double>> points;
    for (double Mt = 90.0; Mt <= 200.0; Mt += 0.25) {
        for (double Mh_input = 0.0; Mh_input <= 100.0; Mh_input += 0.5) {
    ...
    cout << "Analyzing diagnostic points " << start_idx << " to " << end_idx << "..." << endl;
    
    for (size_t i = start_idx; i < end_idx; ++i) {
        double Mt = points[i].first;
        double Mh_input = points[i].second;
        
        auto res = classify_stability_diagnostic(Mh_input, Mt);
        int stability = std::get<0>(res);
        double S_exact = std::get<1>(res);
        double S_approx = std::get<2>(res);
        double Mh_calc = std::get<3>(res);
        double S_kin = std::get<4>(res);
        double S_pot = std::get<5>(res);
        double lambda_R = std::get<6>(res);
        double R_opt = std::get<7>(res);
        
        file << Mt << "," << Mh_calc << "," << stability << "," 
             << S_exact << "," << S_approx << ","
             << S_kin << "," << S_pot << "," << lambda_R << "," << R_opt << "\n";
