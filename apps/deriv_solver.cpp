...
struct PotFD {
    ...
    double dV_ana(RGEHelper& rge, double phi) {
        ...
        // Correct analytic derivative: dV/dphi = phi^3 * (lambda + beta_lambda/4)
        return std::pow(phi, 3) * (lambda + 0.25 * beta_lambda);
    }
    
    void init(RGEHelper& rge, double min_p, double max_p, int N, bool use_analytic) {
        min_phi = min_p;
        ...
        for (int i=0; i<N; ++i) {
            ...
            dV_vals[i] = use_analytic ? dV_ana(rge, phi) : dV_fd(rge, phi);
        }
    }
...
double shoot(const PotFD& pot, double phi_0, double& S_kin, double& S_pot, std::vector<std::pair<double,double>>& profile) {
    State y = {phi_0, 0.0};
    double r = 1e-4;
    double max_r = 1e8;
    S_kin = 0; S_pot = 0;
    profile.clear();
    profile.push_back({r, y.phi});
    ...
    while (r < max_r) {
        ...
        if (std::abs(y.dphi) < 1e-10) dr_target = 1e-4; // FIX!
        ...
        step_cnt++;
        if (step_cnt % 100 == 0) profile.push_back({r, y.phi});
    }
    return y.phi;
}

double find_root(const PotFD& pot, double phi_low, double phi_high, double& S_kin, double& S_pot, std::vector<std::pair<double,double>>& profile) {
    bool found_overshoot = false;
    double d1, d2; std::vector<std::pair<double,double>> d3;
    while(phi_high <= 1e7) {
        double res = shoot(pot, phi_high, d1, d2, d3);
        if (res < 100.0 && res != -1.0) { found_overshoot = true; break; }
    ...
    for (int iter = 0; iter < 100; ++iter) {
        ...
        double res = shoot(pot, phi_opt, S_kin, S_pot, profile);
        if (res < 100.0) phi_high = phi_opt;
        else phi_low = phi_opt;
        if (std::abs(phi_high - phi_low) / phi_opt < 1e-13) break;
    }
    shoot(pot, phi_opt, S_kin, S_pot, profile);
    return S_kin + S_pot;
}

int main(int argc, char* argv[]) {
    RGEHelper rge;
    Params y = get_nnlo_matching(5.0, 105.0);
    double t = 2 * std::log(105.0);
    double dt = 0.1;
    rge.add_point(t, y);
    while (t < 2*std::log(1.22e19)) {
        if (t + dt > 2*std::log(1.22e19)) dt = 2*std::log(1.22e19) - t;
        y = rk4_single_step(y, t, dt);
        t += dt;
        rge.add_point(t, y);
    }
    
    PotFD pot_fd, pot_ana;
    pot_fd.init(rge, 100.0, 1e18, 1000000, false);
    pot_ana.init(rge, 100.0, 1e18, 1000000, true);
    
    double S_kin_fd, S_pot_fd;
    std::vector<std::pair<double,double>> profile_fd;
    double S_tot_fd = find_root(pot_fd, 1e5, 1e6, S_kin_fd, S_pot_fd, profile_fd);
    
    double S_kin_ana, S_pot_ana;
    std::vector<std::pair<double,double>> profile_ana;
    double S_tot_ana = find_root(pot_ana, 1e5, 1e6, S_kin_ana, S_pot_ana, profile_ana);
    
    std::cout << "FD," << S_tot_fd << "," << S_kin_fd << "," << S_pot_fd << "\n";
    std::cout << "Analytic," << S_tot_ana << "," << S_kin_ana << "," << S_pot_ana << "\n";
    
    std::ofstream fprof_fd("data/profile_fd.csv");
    for(auto p : profile_fd) fprof_fd << p.first << "," << p.second << "\n";
    std::ofstream fprof_ana("data/profile_ana.csv");
    for(auto p : profile_ana) fprof_ana << p.first << "," << p.second << "\n";
    
    std::ofstream fdv("data/deriv_comparison.csv");
    fdv << "phi,dV_fd,dV_ana\n";
    for(int i=0; i<1000; i++) {
        double phi = std::exp(std::log(100.0) + i*(std::log(1e18)-std::log(100.0))/999.0);
        double v1, d1, v2, d2;
        pot_fd.eval(phi, v1, d1);
        pot_ana.eval(phi, v2, d2);
        fdv << phi << "," << d1 << "," << d2 << "\n";
    }
