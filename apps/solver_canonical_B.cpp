...
struct CanonicalPot {
    double c6;
    RGEHelper* rge;
    double Mh_val = 5.0; // We'll assume Mh=5 for this specific test
    ...
    double V(double phi) const {
        if (phi > v_ew) {
            ...
            double t_v = 2.0 * std::log(v_ew);
            Params p_v = rge->get_params(t_v);
            double lam_v = get_lambda_eff(p_v);
            double b_lam_v = betaLambda(p_v);
            double V_v = 0.25 * lam_v * std::pow(v_ew, 4) + (c6 / 6.0) * std::pow(v_ew, 6) / (MPlanck * MPlanck);
            double dV_v = std::pow(v_ew, 3) * (lam_v + 0.25 * b_lam_v) + c6 * std::pow(v_ew, 5) / (MPlanck * MPlanck);
            return V_v + dV_v * (phi - v_ew) + 0.5 * Mh_val * Mh_val * std::pow(phi - v_ew, 2);
        }
    }
    ...
    double dV(double phi) const {
        if (phi > v_ew) {
            ...
            double b_lam_v = betaLambda(p_v);
            double dV_v = std::pow(v_ew, 3) * (lam_v + 0.25 * b_lam_v) + c6 * std::pow(v_ew, 5) / (MPlanck * MPlanck);
            return dV_v + Mh_val * Mh_val * (phi - v_ew);
        }
    }
};
