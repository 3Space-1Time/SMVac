...
int main(int argc, char* argv[]) {
    ...
    std::ofstream out("data/epsilon_test_results.csv");
    out << "epsilon,phi_0,taylor_corr,first_phi,first_dphi,overflow,step_cnt,status,max_r,S_kin,S_pot,max_residual\n";
    
    std::vector<double> epsilons = {1e-1, 1e-2, 1e-3, 1e-4};
    std::vector<double> phis = {1e10, 1e12};
    
    for (double eps : epsilons) {
        for (double phi0 : phis) {
            TrajectoryResult r = shoot(pot, phi0, eps);
            out << std::scientific << eps << "," << phi0 << "," << r.taylor_corr << ","
                << r.first_step_phi << "," << r.first_step_dphi << "," << (r.overflow ? "true" : "false") << ","
                << r.step_cnt << "," << status_to_string(r.status) << "," << r.max_r << ","
                << r.S_kin << "," << r.S_pot << "," << r.max_residual << "\n";
            
            std::cout << "eps=" << eps << " phi0=" << phi0 << " -> status=" << status_to_string(r.status) 
                      << " steps=" << r.step_cnt << " max_r=" << r.max_r << std::endl;
        }
    }
