...
private:
    ...
    double t_match;
    double m2;
    vector<pair<double, Params>> history;
...
public:
    PotentialEvaluator(double Mh, double Mt) {
        ...
        // Build dense history from 1.0 to 1e19
        double t_start = 2.0 * log(1.0);
        double t_end = 2.0 * log(1e19);
        
        Params y_rev = y_match;
        double t = t_match;
        double dt = -0.001;
        while (t > t_start) {
            if (t + dt < t_start) dt = t_start - t;
            y_rev = rk4_single_step(y_rev, t, dt);
            t += dt;
            history.push_back({t, y_rev});
        }
        
        Params y_fwd = y_match;
        t = t_match;
        dt = 0.001;
        history.push_back({t, y_fwd});
        while (t < t_end) {
            if (t + dt > t_end) dt = t_end - t;
            y_fwd = rk4_single_step(y_fwd, t, dt);
            t += dt;
            history.push_back({t, y_fwd});
        }
        std::sort(history.begin(), history.end(), [](auto& a, auto& b) { return a.first < b.first; });
    }
    
    Params get_params(double phi) {
        double t = 2.0 * log(phi);
        auto it = upper_bound(history.begin(), history.end(), t, 
            [](double val, const pair<double, Params>& elem){ return val < elem.first; });
        if (it == history.begin()) return history.front().second;
        if (it == history.end()) return history.back().second;
        const auto& p1 = *(it - 1);
        const auto& p2 = *it;
        double factor = (t - p1.first) / (p2.first - p1.first);
        Params res = p1.second;
        res.lambda = p1.second.lambda + factor * (p2.second.lambda - p1.second.lambda);
        res.yt = p1.second.yt + factor * (p2.second.yt - p1.second.yt);
        res.g1 = p1.second.g1 + factor * (p2.second.g1 - p1.second.g1);
        res.g2 = p1.second.g2 + factor * (p2.second.g2 - p1.second.g2);
        res.g3 = p1.second.g3 + factor * (p2.second.g3 - p1.second.g3);
        res.yb = p1.second.yb + factor * (p2.second.yb - p1.second.yb);
        res.ytau = p1.second.ytau + factor * (p2.second.ytau - p1.second.ytau);
        return res;
    }
    
    double V_solver(double phi) {
        Params p = get_params(phi);
        return 0.25 * get_lambda_eff(p) * pow(phi, 4);
    }
    
    double dV_dphi_solver(double phi) {
        Params p = get_params(phi);
        return pow(phi, 3) * (get_lambda_eff(p) + 0.5 * get_beta_eff_analytic(p));
    }
    
    double V_full(double phi) {
        Params p = get_params(phi);
        return -0.5 * m2 * phi * phi + 0.25 * get_lambda_eff(p) * pow(phi, 4);
    }
    
    double dV_dphi_full(double phi) {
        Params p = get_params(phi);
        return -m2 * phi + pow(phi, 3) * (get_lambda_eff(p) + 0.5 * get_beta_eff_analytic(p));
    }
    
    double d2V_dphi2_full(double phi) {
        Params p = get_params(phi);
        ...
        double h = 1e-5;
        double t = 2.0 * log(phi);
        auto eval_beta = [&](double t_eval) {
            auto it = upper_bound(history.begin(), history.end(), t_eval, 
                [](double val, const pair<double, Params>& elem){ return val < elem.first; });
            if (it == history.begin()) return get_beta_eff_analytic(history.front().second);
            if (it == history.end()) return get_beta_eff_analytic(history.back().second);
            const auto& p1 = *(it - 1); const auto& p2 = *it;
            double factor = (t_eval - p1.first) / (p2.first - p1.first);
            Params res = p1.second;
            res.lambda = p1.second.lambda + factor * (p2.second.lambda - p1.second.lambda);
            res.yt = p1.second.yt + factor * (p2.second.yt - p1.second.yt);
            res.g1 = p1.second.g1 + factor * (p2.second.g1 - p1.second.g1);
            res.g2 = p1.second.g2 + factor * (p2.second.g2 - p1.second.g2);
            return get_beta_eff_analytic(res);
        };
        double b_beta = (eval_beta(t + h) - eval_beta(t - h)) / (2.0 * h);
        
        return -m2 + phi * phi * (3.0 * lam_eff + 3.5 * b_eff + b_beta);
    }
    
    double d2V_dphi2_solver(double phi) {
        return d2V_dphi2_full(phi) + m2;
    }
};

double find_root_bisection(PotentialEvaluator& eval, double a, double b, bool use_full) {
    double fa = use_full ? eval.dV_dphi_full(a) : eval.dV_dphi_solver(a);
    double fb = use_full ? eval.dV_dphi_full(b) : eval.dV_dphi_solver(b);
    if (fa * fb > 0) return -1.0; 
    
    for (int i = 0; i < 100; ++i) {
        double c = 0.5 * (a + b);
        double fc = use_full ? eval.dV_dphi_full(c) : eval.dV_dphi_solver(c);
...
int main() {
    ...
    ofstream res("data/verify_stationary_points.csv");
    res << "Model,Point,phi,V,dV_dphi,d2V_dphi2,Classification\n";
    
    auto log_point = [&](string model, string name, double phi, bool use_full) {
        if (phi <= 0) return;
        double v = use_full ? eval.V_full(phi) : eval.V_solver(phi);
        double dv = use_full ? eval.dV_dphi_full(phi) : eval.dV_dphi_solver(phi);
        double d2v = use_full ? eval.d2V_dphi2_full(phi) : eval.d2V_dphi2_solver(phi);
        string cls = (d2v > 0) ? "Minimum" : "Maximum";
        res << model << "," << name << "," << std::scientific << setprecision(12) 
            << phi << "," << v << "," << dv << "," << d2v << "," << cls << "\n";
    };
    
    log_point("Full", "EW_Minimum", find_root_bisection(eval, 10.0, 150.0, true), true);
    log_point("Full", "Barrier_Maximum", find_root_bisection(eval, 200.0, 300.0, true), true);
    log_point("Full", "High_Field_Minimum", find_root_bisection(eval, 5e5, 5e6, true), true);
    
    // For solver, EW minimum is exactly 0. Root bisection will find barrier.
    log_point("Solver", "EW_Minimum", 0.0, false); // V=0 at phi=0
    log_point("Solver", "Barrier_Maximum", find_root_bisection(eval, 200.0, 300.0, false), false);
    log_point("Solver", "High_Field_Minimum", find_root_bisection(eval, 5e5, 5e6, false), false);
    
    res.close();
    
    ofstream grid("data/potential_verification_grid.csv");
    grid << "phi,V_solver,dV_solver,d2V_solver,V_full,dV_full,d2V_full\n";
    grid << std::scientific << setprecision(12);
    // dense log-spaced grid
    for(int i=0; i<=10000; ++i) {
        double phi = pow(10.0, 2.0 + 17.0 * i / 10000.0);
        grid << phi << "," << eval.V_solver(phi) << "," << eval.dV_dphi_solver(phi) << "," << eval.d2V_dphi2_solver(phi)
             << "," << eval.V_full(phi) << "," << eval.dV_dphi_full(phi) << "," << eval.d2V_dphi2_full(phi) << "\n";
    }
    grid.close();
    
    cout << "Verification Grid and Stationary points dumped successfully.\n";
    return 0;
}
