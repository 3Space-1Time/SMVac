...
TrajectoryResult shoot(const CanonicalPot& pot, double phi_0, double tol_dr=0.01, int max_steps=1000000) {
    ...
    std::cout << "step,r,phi,dphi,dV,dr,truncation_error,friction" << std::endl;
    
    while (r < 1e12) {
        step_cnt++;
        if (step_cnt > max_steps) {
            std::cout << "DEBUG: Terminating due to step_cnt > max_steps" << std::endl;
            return {MAX_STEPS, 0, 0, y.phi}; // Return 0 action for non-converged
        }

        
        // RK4 Step function
        auto do_rk4 = [&](double curr_r, State curr_y, double step_dr) -> State {
            auto derivs = [&](double r_eval, State y_eval) -> State {
                double V_val = pot.V(y_eval.phi);
                double dV_val = pot.dV(y_eval.phi);
                double friction = (r_eval == 0) ? 0 : (3.0 / r_eval) * y_eval.dphi;
                return { y_eval.dphi, -friction + dV_val };
            };
            State k1 = derivs(curr_r, curr_y);
            State k2 = derivs(curr_r + 0.5*step_dr, {curr_y.phi + 0.5*step_dr*k1.phi, curr_y.dphi + 0.5*step_dr*k1.dphi});
            State k3 = derivs(curr_r + 0.5*step_dr, {curr_y.phi + 0.5*step_dr*k2.phi, curr_y.dphi + 0.5*step_dr*k2.dphi});
            State k4 = derivs(curr_r + step_dr, {curr_y.phi + step_dr*k3.phi, curr_y.dphi + step_dr*k3.dphi});
            return {
                curr_y.phi + (step_dr/6.0)*(k1.phi + 2*k2.phi + 2*k3.phi + k4.phi),
                curr_y.dphi + (step_dr/6.0)*(k1.dphi + 2*k2.dphi + 2*k3.dphi + k4.dphi)
            };
        };
        
        // One full step
        State y_next = do_rk4(r, y, dr);
        // Two half steps
        State y_half = do_rk4(r, y, 0.5*dr);
        State y_full = do_rk4(r + 0.5*dr, y_half, 0.5*dr);
        
        double trunc_err = std::abs(y_next.phi - y_full.phi) + std::abs(y_next.dphi - y_full.dphi);
        
        // Use the more accurate full step
        y_next = y_full;
        double dV_curr = pot.dV(y.phi);
        double friction = (r == 0) ? 0 : (3.0 / r) * y.dphi;
        if (true) {
            std::cout << step_cnt << "," << r << "," << y.phi << "," << y.dphi << "," 
                     << dV_curr << "," << dr << "," << trunc_err << "," << friction << std::endl;
        }
        
        if (std::isnan(dr)) {
            std::cout << "DEBUG: dr is NaN! y.phi=" << y.phi << " y.dphi=" << y.dphi << std::endl;
        }
        ...
        if (std::isnan(y.phi) || std::isnan(y.dphi)) {
            std::cout << "DEBUG: Terminating due to NaN. y.phi=" << y.phi << ", y.dphi=" << y.dphi << std::endl;
            return {FAILED, 0, 0, y.phi};
        }
        if (y.phi < 0) {
            std::cout << "DEBUG: Terminating due to OVERSHOOT. y.phi < 0" << std::endl;
            return {OVERSHOOT, 0, 0, y.phi}; // Overshoot crosses 0
        }
        if (y.dphi > 0) {
            // If it undershoots near the vacuum, it's considered converged!
            if (y.phi < 1000.0) {
                std::cout << "DEBUG: Terminating due to CONVERGED. y.phi=" << y.phi << std::endl;
                return {CONVERGED, S_kin, S_pot, y.phi};
            }
            std::cout << "DEBUG: Terminating due to UNDERSHOOT. y.dphi > 0" << std::endl;
    ...
    std::cout << "DEBUG: Reached end of shoot. r=" << r << " step_cnt=" << step_cnt << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Starting diagnostic shoot..." << std::endl;
    TrajectoryResult final_res = shoot(pot, 10000.0, 0.01, 100000);
    std::cout << "Done. Final Status: " << status_to_string(final_res.status) << std::endl;
