...
def run_chunk(start, end):
    ...
    subprocess.run(["solver_numerical.exe", str(start), str(end)])
...
all_files = glob.glob("results/data_numerical_chunk_*.csv")
df_list = []
for f in all_files:
    df_list.append(pd.read_csv(f))
    
combined = pd.concat(df_list, ignore_index=True)
combined.to_csv("results/numerical_data.csv", index=False)
