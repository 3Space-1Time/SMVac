...
// --- RGE HELPER WITH OPTIMIZED O(1) LOOKUP ---
class RGEHelper {
private:
...
public:
    ...
    Params get_params(double t) const {
        ...
        // Since dt is exactly 0.1 (except the very last chunk), we can estimate the index in O(1)
        int idx = (t - table.front().first) * 10.0;
        if (idx < 0) idx = 0;
        if (idx >= (int)table.size() - 1) idx = (int)table.size() - 2;
        
        // Adjust idx if there are slight floating point variations
        while (idx > 0 && table[idx].first > t) {
            idx--;
        }
        while (idx < (int)table.size() - 1 && table[idx+1].first <= t) {
            idx++;
        }
        
        const auto& p1 = table[idx];
        const auto& p2 = table[idx+1];
...
static constexpr int    N_QUAD     = 512;

static double get_pure_sm_lambda(const RGEHelper& rge, double phi_dimless, double mu_inst) {
...
static double integrand_u(const RGEHelper& rge, double u, double R, double mu_inst, double prefactor) {
...
double evaluate_action_at_R(const RGEHelper& rge, double mu_inst, double R) {
...
double find_minimum_action(const RGEHelper& rge, double mu_inst, double t_min_lambda) {
...
int main(int argc, char* argv[]) {
    // Step size for 64x denser closeup plot (8x per axis compared to 0.25)
    double step = 0.03125;
    
    vector<pair<double,double>> points;
    for (double Mt = 160.0; Mt <= 185.0; Mt += step) {
        for (double Mh_input = 110.0; Mh_input <= 140.0; Mh_input += step) {
            points.emplace_back(Mt, Mh_input);
        }
    }
    
    size_t num_points = points.size();
    vector<Result> results(num_points);
    
    cout << "Total points: " << num_points << endl;
    cout << "Analyzing numerical dense closeup grid using OpenMP..." << endl;
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        results[i] = {
            Mt,
            std::get<3>(res), // Mh_calc
            std::get<0>(res), // stability
            std::get<1>(res), // S_exact
            std::get<2>(res)  // S_approx
        };
        ...
        if (c % 10000 == 0) {
            #pragma omp critical
            {
                cout << "Processed " << c << " / " << num_points << " (" << (c * 100.0 / num_points) << "%)" << endl;
            }
        }
    }
    
    std::string filename = "results/numerical_dense_closeup.csv";
    cout << "Saving results to " << filename << "..." << endl;
    ofstream file(filename);
    file << "Mt,Mh_calc,Stability,S_exact,S_approx\n";
    for (const auto& r : results) {
        file << r.Mt << "," << r.Mh_calc << "," << r.stability << "," << r.S_exact << "," << r.S_approx << "\n";
    }
    file.close();
    
    cout << "Done. Saved all " << num_points << " points." << endl;
    return 0;
}
