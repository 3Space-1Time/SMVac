...
def run_chunk(start, end):
    ...
    subprocess.run(["apps/generate_phase_diagram.exe", "--analytical", "--dense", str(start), str(end)])
...
all_files = glob.glob("results/analytical_data_dense_chunk_*.csv")
df_list = []
for f in all_files:
    df_list.append(pd.read_csv(f))
    
combined = pd.concat(df_list, ignore_index=True)
combined.to_csv("data/analytical_data_dense.csv", index=False)
