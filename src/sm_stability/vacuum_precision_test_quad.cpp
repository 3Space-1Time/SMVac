...
public:
    ...
    qFloat vpot(const qFloat* phi) const override {
        double phi_d = (double)phi[0];
        double h = phi_d * mu_inst;
        if (h <= v) return 0.0Q; 
        double t = 2.0 * log(h); 
        Params p = rge.get_params(t);
        // Normalized V = V_phys / mu^4
        qFloat lambda_q = (qFloat)p.lambda;
        return 0.25Q * lambda_q * powq(phi[0], 4.0Q); 
    }

    void calcDvdphi(const qFloat* phi, qFloat* dvdphi_out) const override {
        double phi_d = (double)phi[0];
        double h = phi_d * mu_inst;
        if (h <= v) { 
            dvdphi_out[0] = 0.0Q; 
        ...
        double t = 2.0 * log(h); 
        Params p = rge.get_params(t);
        // dV/dphi = lambda * phi^3
        qFloat lambda_q = (qFloat)p.lambda;
        dvdphi_out[0] = lambda_q * powq(phi[0], 3.0Q);
    }

    // Helper to check if a point has negative energy
...
void evaluate_point(double Mt, double Mh) {
    ...
    cout << "Mt = " << Mt << ", Mh = " << Mh << " | " << flush;
    if (!is_unstable) { cout << "STABLE!" << endl; return; }
    ...
    if (phi_true_guess > 0) {
        ...
        bounce.setModel(&pot);
        qFloat phi_true_arr[] = {(qFloat)phi_true_guess}; 
        qFloat phi_false_arr[] = {0.0Q};
        bounce.setVacuum(phi_true_arr, phi_false_arr);
        int status = bounce.solve();
        if (status != 0) { cout << "SimpleBounceQuad failed!" << endl; }
        else {
            qFloat exact_action = bounce.action();
            cout << "S_exact = " << (double)exact_action << endl;
        }
    } else {
        cout << "Could not find true vacuum!" << endl;
    }
}

int main() {
    cout << "--- Exact Action Calculation (Quad Precision 128-bit) ---" << endl;
    evaluate_point(173.1, 125.1);
    evaluate_point(173.1, 125.2);
    evaluate_point(173.2, 125.1);
    evaluate_point(173.2, 125.2);
    return 0;
}
